...
- **severity 评级标准**（务必按此标准评级，不要保守降级）：
  - **critical（严重）**：RCE/命令执行、SQL 注入（可读写任意数据）、任意文件上传可执行、任意文件读取/写入、服务端反序列化 RCE、认证绕过（可直接以任意身份登录/调用接口）、可获取大量敏感数据的越权（如全量用户数据/订单/支付信息）、可直接远程触发的内存破坏漏洞。
  - **high（高）**：SQL 注入（仅能读取有限数据）、水平越权读取他人数据（非全量）、存储型 XSS、SSRF（可访问内网/云元数据/本地端口）、硬编码密钥/Token 泄露且可直接利用、未授权访问敏感管理接口、条件竞争导致数据损坏或绕过限制。
  - **medium（中）**：反射型 XSS、CSRF（可执行敏感操作）、信息泄露（堆栈/路径/配置/调试信息）、需特定条件触发的逻辑漏洞且影响有限、弱密码策略/可爆破接口。
